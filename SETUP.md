...
**First — use VLC (on any system) to verify the teletext stream is
actually there:**
...
VLC lists all elementary streams including their PIDs. Look for a stream
described as "Teletext" or "DVB Teletext". This step also confirms the
channel is broadcasting teletext before you go any further.

**Then — get the exact PID value with ffprobe:**
...
On Windows (if you want to keep the Linux box clean): download the
essentials build from [gyan.dev/ffmpeg/builds](https://www.gyan.dev/ffmpeg/builds/),
extract it — no installer needed — and run from PowerShell or cmd:
```powershell
...
```
Convert the hex PID to decimal: `0x199` = **409** — that is the value
you pass to the service.
